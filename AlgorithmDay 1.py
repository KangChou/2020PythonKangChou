...
#这里的四个数字从1~10数字中抽取四个数字，可以用棋牌的数为实物，A代表1，JQK去掉，大王小王爷去掉，只留数字（含A）。
def twentyfour(cards):
    '''史上最短计算24点代码'''
    for nums in itertools.permutations(cards):  # permutations排列四个数,全排列
        for ops in itertools.product('+-*/', repeat=3):  # 三个运算符（可重复！）
            ...
            for bds in [bds1, bds2, bds3]:  # 遍历
                try:
                    if abs(eval(bds) - 10.0) < 1e-10:  # eval函数  #1e-10表示精度1*10^-10
                        return bds
